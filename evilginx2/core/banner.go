...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Latest Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Dev Brainiac (") + handleClr.Sprintf("@devbrainiac") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
