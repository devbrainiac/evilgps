    ...
    api.campaignId.results(campaign.id)
        .success(function (c) {
            ...
            resultsTable.rows().every(function (i, tableLoop, rowLoop) {
                ...
                var keyname = rowData[0]
                $.each(campaign.results, function (j, result) {
                    if (result.id == keyname) {
...
function report_mail(keyname, cid) {
    Swal.fire({
        title: "Are you sure?",
        text: "This result will be flagged as reported (RID: " + keyname + ")",
        type: "question",
        animation: false,
        showCancelButton: true,
        ...
        if (result.value){
            api.campaignId.get(cid).success((function(c) {
                ...
                report_url.search = "?keyname=" + keyname 
                fetch(report_url)
                .then(response => {
                    if (!response.ok) {
