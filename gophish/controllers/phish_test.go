...
func openEmail(t *testing.T, ctx *testContext, keyname string) {
	resp, err := http.Get(fmt.Sprintf("%s/track?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
...
func openEmail404(t *testing.T, ctx *testContext, keyname string) {
	resp, err := http.Get(fmt.Sprintf("%s/track?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
		t.Fatalf("error requesting /track endpoint: %v", err)
	}
...
func reportedEmail(t *testing.T, ctx *testContext, keyname string) {
	resp, err := http.Get(fmt.Sprintf("%s/report?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
...
func reportEmail404(t *testing.T, ctx *testContext, keyname string) {
	resp, err := http.Get(fmt.Sprintf("%s/report?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
		t.Fatalf("error requesting /report endpoint: %v", err)
	}
...
func clickLink(t *testing.T, ctx *testContext, keyname string, expectedHTML string) {
	resp, err := http.Get(fmt.Sprintf("%s/?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
...
func clickLink404(t *testing.T, ctx *testContext, keyname string) {
	resp, err := http.Get(fmt.Sprintf("%s/?%s=%s", ctx.phishServer.URL, models.RecipientParameter, keyname))
	if err != nil {
		t.Fatalf("error requesting / endpoint: %v", err)
	}
...
func transparencyRequest(t *testing.T, ctx *testContext, r models.Result, keyname, path string) {
	resp, err := http.Get(fmt.Sprintf("%s%s?%s=%s", ctx.phishServer.URL, path, models.RecipientParameter, keyname))
	if err != nil {
...
func TestInvalidRecipientID(t *testing.T) {
	...
	defer tearDown(t, ctx)
	keyname := "XXXXXXXXXX"
	openEmail404(t, ctx, keyname)
	clickLink404(t, ctx, keyname)
	reportEmail404(t, ctx, keyname)
}
...
func TestTransparencyRequest(t *testing.T) {
	...
	campaign := getFirstCampaign(t)
	result := campaign.Results[0]
	keyname := fmt.Sprintf("%s%s", result.RId, TransparencySuffix)
	transparencyRequest(t, ctx, result, keyname, "/")
	transparencyRequest(t, ctx, result, keyname, "/track")
	transparencyRequest(t, ctx, result, keyname, "/report")

	// And check with the URL encoded version of a +
	keyname = fmt.Sprintf("%s%s", result.RId, "%2b")
	transparencyRequest(t, ctx, result, keyname, "/")
	transparencyRequest(t, ctx, result, keyname, "/track")
	transparencyRequest(t, ctx, result, keyname, "/report")
}
