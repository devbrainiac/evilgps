...
func (ps *PhishingServer) TrackHandler(w http.ResponseWriter, r *http.Request) {
	...
	keyname := ctx.Get(r, "keyname").(string)
	d := ctx.Get(r, "details").(models.EventDetails)

	// Check for a transparency request
	if strings.HasSuffix(keyname, TransparencySuffix) {
...
func setupContext(r *http.Request) (*http.Request, error) {
	...
	keyname := r.Form.Get(models.RecipientParameter)
	if keyname == "" {
	...
	if strings.HasSuffix(keyname, " ") {
		...
		keyname = fmt.Sprintf("%s%s", keyname, TransparencySuffix)
	}
	// Finally, if this is a transparency request, we'll need to verify that
	// a valid keyname has been provided, so we'll look up the result with a
	// trimmed parameter.
	id := strings.TrimSuffix(keyname, TransparencySuffix)
