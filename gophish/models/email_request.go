...
// PreviewPrefix is the standard prefix added to the keyname parameter when sending
// test emails.
...
func PostEmailRequest(s *EmailRequest) error {
	...
	keyname, err := generateResultId()
	if err != nil {
		return err
	}
	s.RId = fmt.Sprintf("%s%s", PreviewPrefix, keyname)
	return db.Save(&s).Error
}

// GetEmailRequestByResultId retrieves the EmailRequest by the underlying keyname
// parameter.
