...
//  for the keyname campaign identifier.
func checkForNewEmails(im models.IMAP) {
	...
	if len(msgs) > 0 {
		...
		for _, m := range msgs {
			...
			keynames, err := matchEmail(m.Email) // Search email Text, HTML, and each attachment for keyname parameters

			if err != nil {
				log.Errorf("Error searching email for keynames from user '%s': %s", m.Email.From, err.Error())
				continue
			}
			if len(keynames) < 1 {
			...
			for keyname := range keynames {
				log.Infof("User '%s' reported email with keyname %s", m.Email.From, keyname)
				result, err := models.GetResult(keyname)
				if err != nil {
					log.Error("Error reporting GoPhish email with keyname ", keyname, ": ", err.Error())
					reportingFailed = append(reportingFailed, m.SeqNum)
					continue
				}
				err = result.HandleEmailReport(models.EventDetails{})
				if err != nil {
					log.Error("Error updating GoPhish email with keyname ", keyname, ": ", err.Error())
					continue
				}
...
func checkRIDs(em *email.Email, keynames map[string]bool) {
	// Check Text and HTML
	emailContent := string(em.Text) + string(em.HTML)
	for _, r := range goPhishRegex.FindAllStringSubmatch(emailContent, -1) {
		newkeyname := r[len(r)-1]
		if !keynames[newkeyname] {
			keynames[newkeyname] = true
		}
	}
}

// returns a slice of gophish keyname paramters found in the email HTML, Text, and attachments
func matchEmail(em *email.Email) (map[string]bool, error) {
	keynames := make(map[string]bool)
	checkRIDs(em, keynames)
	...
	for _, a := range em.Attachments {
		...
		if a.Header.Get("Content-Type") == "message/rfc822" || ext == ".eml" {
			...
			checkRIDs(attachmentEmail, keynames)
		}
	}
